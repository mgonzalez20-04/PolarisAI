...
export async function GET() {
  ...
  try {
    // Get agent database config from AppSettings
    ...
    console.error("Error fetching agent database config:", error);
    return NextResponse.json(
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Save agent database config to AppSettings
    await prisma.appSettings.upsert({
      where: { key: "AGENT_DATABASE_CONFIG" },
      update: { value: JSON.stringify(config), updatedAt: new Date() },
      create: { key: "AGENT_DATABASE_CONFIG", value: JSON.stringify(config) },
    });
